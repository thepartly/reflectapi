...
fn update_type_references_in_output_type(
    output_type: &mut reflectapi_schema::OutputType,
    name_mapping: &HashMap<String, String>,
) {
    match output_type {
        reflectapi_schema::OutputType::Complete { output_type } => {
            update_type_reference_in_option(output_type, name_mapping);
        }
        reflectapi_schema::OutputType::Stream { item_type } => {
...
impl Normalizer {
    ...
    fn resolve_function_references(&mut self, function_id: &SymbolId, function: &Function) {
        ...
        match &function.output_type {
            reflectapi_schema::OutputType::Complete {
            ...
            reflectapi_schema::OutputType::Complete { output_type: None } => {}
        }
    ...
    fn build_semantic_function(
        ...
        let output_type = match &function.output_type {
            reflectapi_schema::OutputType::Complete { output_type } => {
                SemanticOutputType::Complete(
                    output_type
                        .as_ref()
                        .and_then(|tr| self.resolve_global_type_reference(&tr.name)),
                )
            }
            reflectapi_schema::OutputType::Stream { item_type } => SemanticOutputType::Stream {
                item_type: self
                    .resolve_global_type_reference(&item_type.name)
                    .ok_or_else(|| {
                        vec![NormalizationError::UnresolvedReference {
                            name: item_type.name.clone(),
                            referrer: SymbolId::new(
                                SymbolKind::Endpoint,
...
mod tests {
    ...
    fn test_function_symbol_path_matches_id() {
        ...
        function.output_type = reflectapi_schema::OutputType::Complete { output_type: None };
        schema.functions.push(function);
