...
test.describe("Err class with HTTP status codes", () => {
  ...
  test("toString includes status information", () => {
    const metadata1: TransportMetadata = {
      status: 400,
      headers: {},
      timing: undefined,
      raw: undefined,
    };
    const metadata2: TransportMetadata = {
      status: 500,
      headers: {},
      timing: undefined,
      raw: undefined,
    };
    const appErr = new Err({
      application_err: "ValidationFailed",
      metadata: metadata1,
    });
    const otherErr = new Err({
      other_err: "Server Error",
      metadata: metadata2,
    });
...
test.describe("__request function with status codes", () => {
  test("success response - backward compatible API", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(201, '{"result": "created"}')]]),
    );
    ...
    // Test backward compatible access - no .value needed!
    const value = result.unwrap_ok();
    expect(value).toEqual({ result: "created" });
  ...
  test("not found error (404)", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(404, '{"error": "not_found"}')]]),
    );
  ...
  test("conflict error (409)", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(409, '{"error": "conflict"}')]]),
    );
  ...
  test("server error (500+)", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(500, "Internal Server Error")]]),
    );
  ...
  test("bad gateway error (502)", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(502, "Bad Gateway")]]),
    );
  ...
  test("invalid JSON in success response falls back to other_err with status", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(200, "invalid json")]]),
    );
  ...
  test("invalid JSON in application error falls back to other_err", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(400, "invalid json")]]),
    );
...
test.describe("Backward Compatibility", () => {
  test("existing error handling patterns still work", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(400, '{"error": "test_error"}')]]),
    );
  ...
  test("existing client usage patterns unchanged", async () => {
    const client = new MockClient(
      new Map([["/test", createMockResponse(409, '"Conflict"')]]),
    );
...
test.describe("Enhanced Error Handling Use Cases", () => {
  test("can handle different status codes for same error type", async () => {
    const client400 = new MockClient(
      new Map([["/test", createMockResponse(400, '"ValidationError"')]]),
    );
    const client422 = new MockClient(
      new Map([["/test", createMockResponse(422, '"ValidationError"')]]),
    );
  ...
  test("comprehensive error handling with status-based logic", async () => {
    ...
    for (const [statusCode, response] of scenarios) {
      const client = new MockClient(
        new Map([["/test", createMockResponse(statusCode, response)]]),
      );
      const result = await __request(client, "/test", {}, {});
