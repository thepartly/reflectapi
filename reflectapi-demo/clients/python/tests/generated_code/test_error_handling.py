...
class TestModelValidation:
    ...
    def test_behavior_invalid_enum_value(self):
        ...
        errors = exc_info.value.errors()
        assert any('enum' in error['type'] for error in errors)
...
class TestErrorEnums:
    ...
    def test_pets_create_error_values(self):
        """Test PetsCreateError enum values."""
        assert PetsCreateError.CONFLICT == "Conflict"
        assert PetsCreateError.NOT_AUTHORIZED == "NotAuthorized"
        assert PetsCreateError.INVALID_IDENTITY == "InvalidIdentity"
        
        # Test all values are strings
        for error_type in PetsCreateError:
...
class TestEdgeCases:
    ...
    def test_pet_duplicate_behaviors(self):
        ...
        pet = Pet(
            name="Test",
            kind=PetKindDog(type='dog', breed='Labrador'),
            behaviors=[Behavior.CALM, Behavior.CALM, Behavior.AGGRESSIVE]
        )
        # Duplicates should be preserved
        assert pet.behaviors == [Behavior.CALM, Behavior.CALM, Behavior.AGGRESSIVE]
