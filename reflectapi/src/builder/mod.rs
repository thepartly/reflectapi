    ...
    S: Send + 'static,
{
    ...
    /// Registers shared modules for a specific codegen language.
    ///
    /// Types from these modules are assumed to already exist as dependencies and
    /// will not be generated by the codegen. This information is stored in the
    /// schema and is available to validation steps.
    ///
    /// # Example
    ///
    /// ```ignore
    /// builder.shared_modules("rust", ["tier1_assemblies_m", "tier1_vehicles_m"])
    /// ```
    pub fn shared_modules(
        mut self,
        ...
        self.schema
            ...
            .entry(language.into())
            .or_default()
            .extend(modules.into_iter().map(|m| m.into()));
        self
    }
