...
from reflectapi_runtime import (
    ClientBase, AsyncClientBase, ApiResponse, ReflectapiOption, Undefined,
    ApplicationError, NetworkError, TimeoutError, ValidationError as ReflectApiValidationError,
    TransportMetadata, BearerTokenAuth, APIKeyAuth, BasicAuth
)
...
class TestClientEdgeCases:
    ...
    def test_request_with_both_json_data_and_model(self, mock_send):
        ...
        with pytest.raises(ValueError, match="Cannot specify both json_data and json_model"):
            client._make_request("POST", "/test", json_data={"key": "value"}, json_model=model)
...
class TestErrorHandlingEdgeCases:
    ...
    def test_response_with_invalid_json_body(self, mock_send):
        ...
        with pytest.raises(ReflectApiValidationError, match="Failed to parse JSON response"):
            client._make_request("GET", "/test")
