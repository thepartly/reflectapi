...
class Response:
    ...
    headers: Any
    body: bytes
    # Optional escape hatch for adapters that have a richer wire-level
    # response object to surface (e.g. an httpx.Response with `.request`
    # / `.extensions` / `.history` populated). The runtime uses this to
    # populate `TransportMetadata.raw_response` with the real wire object
    # rather than a synthetic stand-in. Middleware should ignore it.
    # Excluded from equality / repr because it's an opaque side channel.
